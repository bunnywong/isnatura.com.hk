...
#historyChartImg_full{ max-width: 100%; height: auto; width:100%; display:none;}	/*	Chart img	*/
/* debug */
#historyChartImg_full{display:none;}
/* debug */
/* #historyChartImg, .historyPoint{display:none;}				
#historyChart{border:1px solid pink; position:relative;} */

.historyDescription{border:1px solid lightgray; border-radius:10px; margin-top:50px; margin-right:auto;
	margin-left:auto; opacity:1; padding:50px; text-align:justify;width:auto;}	/*	Hide Year description	*/
...
#historyContent{padding-right:20px;}
...
<div id="page_history" class="page_content">
	...
	<img src="<?php iszh('../'); ?>includes/img/historyDotChart_full.png" id="historyChartImg_full">
	</div>
	<div id="historyPoint">
...
<div id="historyContent">
	...
	<div class="year2013 historyHide">
		<h3>2013</h3>
		<p><?php ec('I.S. Natura (H.K.) Limited began distributing hair growth products and Avantari Fulvic Acid in Hong Kong and Greater China.','仙源（香港）有限公司開始在香港和內地售賣護髮產品和 AVANRARI 富里酸。'); ?></p>
	</div>
	</div>
</div>
...
function show_all_content(){
	jQuery('historyShowAll').parent().parent().empty()					// Hide present content
					.append(jQuery('#historyContent').children('div')	// Append all history content
					.slideDown(500).removeClass('historyHide')).wrap('<span class="showedAll"></span>').end()

				.find(jQuery('h3')					// New <h3> & <p> format
					.css('line-height','10px').next('p').css({'margin-bottom':'30px','margin-top':'-10px'}));
}
function btn_show_all(){
	jQuery('.historyShowAll').click(function(){					// Click [Show all] to display all history
			show_all_content();
		});
}// !btn_show_all()
...
function mobile(){	
	jQuery('#historyChart img, .historyPoint').hide();
	jQuery('#historyChart').width('auto').height('auto');	// Responsive img
	jQuery('#historyChartImg_full').show();					// [debug] show it later
	jQuery('.historyDescription').hide();					// Hide selected year content		
	show_all_content();	
}// !mobile()
...
function go_responsive(){
	...
	if(win_width > 980){	// Set 980px to avoid img.year overlay
		desktop();
	}else{
		mobile();
	}
