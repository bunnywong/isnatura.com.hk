...
function fixbar(){
	...
	function subMenu_Product(){
      ...
      var FulvicAcidSet = 'ISNATURA富里酸排毒精華禮盒套裝';
      var hairCare      = '頭髮護理';
...
function page_fulvicSet(){
  var sold = Drupal.t('Sold');
  // Run for Chinese (not English).
  if (Drupal.settings.pathPrefix !== "")
    sold = "售完";
  $('.form-item-quantity > label').text(sold);
}
	...
	if(pathExist('zh')){
		var productArr = [	['avantari-regenerating-shampoo', 'Avantari Regenerating Shampoo', '再生洗髮乳'],
              ...
              ['pure-concentrated-fulvic-acid', 'Pure Concentrated Fulvic Acid', '富里酸精華素'],
							['purified-fulvic-acid-extra-set', 'Purified Fulvic Acid Extra Set', 'ISNATURA富里酸排毒精華禮盒套裝'],
...
//	if(isIndex() == true)	indexCtrl();								// Page.Index
	...
	var product = [
          ...
          'purified-fulvic-acid-extra-set',
					...
					'nissenmedica-heavy-metal-screen-test',
					'3-step-kit'
				];	// Page.Product [Single]
  ...
  if(pathExist('fulvic-acid')) page_haircare();         // Page.Products
	if(pathExist('purified-fulvic-acid-extra-set')) page_fulvicSet(); // ***
...
/*-----------------------ling test----------------------------*/
jQuery(".views-field-commerce-coupon-code").length;
var couponlength = jQuery(".views-field-commerce-coupon-code").length;

if(couponlength){
console.log(couponlength + '!!!!!!!!!!!!!');

/*localStorage.setItem('origal_price','test');
localStorage.setItem('origal_price','test');
...
--------------------	*/
	...
	/* Translate: Page.Register */
	if( jQuery('body').hasClass('page-user-register') && jQuery('body').hasClass('i18n-zh-hant') )
		jQuery('.form-item-captcha-questions-answer-given > div').text('請輸入阿拉怕數目字');
